...
'更新时间:2013/9/29
'1.由于1.4版YJK的wdisp中工况代号有变化，例如工况9变成工况10，则用Mid函数所定义的位置有误；改为Right函数，从字符串末尾开始计算位置
...
'更新时间:2013/7/19/ 10:18
'更新内容:
'1.删除对distribution表格格式的设置
...
'更新内容:
'1.移植PKPM的alpha版
'2.YJK中各工况的关键词位置不一样，将原来统一判断的select case改为单个的if
'3.YJK中层间位移角末尾没有小数点，原有的判断失效；先提取"1/XXXX"，再用MID取其后四位
'4.修正最大层间位移角取值范围，包括风工况
...
Keyword_Disp_Ratio_FEXP = "X+ 规定水平力作用下的楼层最大位移"

Keyword_Disp_Ratio_FEXN = "X- 规定水平力作用下的楼层最大位移"

Keyword_Disp_Ratio_FEY = "Y 方向规定水平力作用下的楼层最大位移"

Keyword_Disp_Ratio_FEYP = "Y+ 规定水平力作用下的楼层最大位移"

Keyword_Disp_Ratio_FEYN = "Y- 规定水平力作用下的楼层最大位移"
...
Do While Not EOF(1)
    ...
    If Firststring_Disp_Ratio_FEXP = Keyword_Disp_Ratio_FEXP Or Right(data, 22) = "X+ 偶然偏心规定水平力作用下的楼层最大位移" Then
            Debug.Print data
            Do While Not EOF(1)
                Line Input #i, data
      ...
      '----------------------------------------------------------读取X-偶然偏心地震作用规定水平力下的位移比、层间位移比
      If Firststring_Disp_Ratio_FEXN = Keyword_Disp_Ratio_FEXN Or Right(data, 22) = "X- 偶然偏心规定水平力作用下的楼层最大位移" Then
            Debug.Print data
            Do While Not EOF(1)
                Line Input #i, data
      ...
      If Firststring_Disp_Ratio_FEY = Keyword_Disp_Ratio_FEY Then
        ...
        If Firststring_Disp_Ratio_FEYP = Keyword_Disp_Ratio_FEYP Or Right(data, 22) = "Y+ 偶然偏心规定水平力作用下的楼层最大位移" Then
            Debug.Print data
            Do While Not EOF(1)
                Line Input #i, data
        ...
        '----------------------------------------------------------读取Y-偶然偏心地震作用规定水平力下的位移比、层间位移比
        If Firststring_Disp_Ratio_FEYN = Keyword_Disp_Ratio_FEYN Or Right(data, 22) = "Y- 偶然偏心规定水平力作用下的楼层最大位移" Then
            Debug.Print data
            Do While Not EOF(1)
                Line Input #i, data
